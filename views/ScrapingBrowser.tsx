...
const ScrapingBrowser: React.FC<Props> = ({ settings, initialQuery, onClearQuery, onChangeView }) => {
    ...
    <div className="h-full flex flex-col space-y-6">
      <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
        ...
        <div className="flex items-center gap-3">
            ...
            <Button onClick={handleExport} disabled={items.length === 0} variant="secondary" className="h-[38px]">
        ...
        <form onSubmit={handleSubmit} className="flex gap-4">
           <div className="relative flex-1">
                ...
                <input
                    ...
                    placeholder={mode === 'general' ? "Search for anything (e.g. sunelec.com, grateful dead)" : "Scrape Query (e.g. collection:nasa)"}
      ...
      <div className="flex-1 bg-gray-800 rounded-xl border border-gray-700 overflow-hidden flex flex-col shadow-inner relative">
        ...
        >
             ...
             <div className="flex flex-col items-center justify-center h-80 text-gray-500 animate-in fade-in">
                ...
                <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-700/50 max-w-md text-sm text-left space-y-3">
                    ...
                    <div className="flex items-start gap-2 text-gray-400">
                        ...
                        <span><strong>Tip:</strong> Try simplifying your keywords or removing special characters.</span>
                    </div>
                    ...
                    {mode === 'scrape' && (
                        <div className="flex items-start gap-2 text-gray-400">
                             ...
                             <div>
                                 ...
                                 <button 
                                    onClick={() => { handleModeChange('general'); performSearch(query, true, 'general'); }}
           ...
           {items.map((item, idx) => (
             <div key={`${item.identifier}-${idx}`} className="bg-gray-900/40 border border-gray-800/50 p-4 rounded-lg flex justify-between items-start hover:bg-gray-900 transition-colors group">
               <div className="min-w-0 pr-4">
                 <div className="flex items-center gap-2 mb-1">
                   <h3 className="font-semibold text-teal-400 truncate max-w-lg" title={item.title}>{item.title || item.identifier}</h3>
                   {item.mediatype && (
                 ...
                 <div className="text-xs text-gray-500 font-mono flex items-center gap-2">
                     ...
                     {item.downloads !== undefined && (
                         <span className="text-gray-600">• {item.downloads.toLocaleString()} downloads</span>
                     )}
                 </div>
                 {item.description && (
               ...
               <div className="flex flex-col items-end gap-2 shrink-0">
                 <span className="text-xs text-gray-500 font-medium">{item.date?.toString().substring(0, 10)}</span>
                 <a 
        ...
        {items.length > 0 && (
            ...
            <span className="flex items-center gap-2">
                ...
                Showing {items.length.toLocaleString()} {totalFound ? `of ~${totalFound.toLocaleString()}` : ''} items
            </span>
